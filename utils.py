...
def get_instance_image(img, bbox, size_z, size_x, context_amount, img_mean=None):
    cx, cy, w, h = xyxy2cxcywh(bbox)
    wc_z = w + context_amount * (w+h)  # w + 2p
    hc_z = h + context_amount * (w+h)  # h + 2p
    s_z = np.sqrt(wc_z * hc_z)  # (w + 2p)(h + 2p)
    scale_z = size_z / s_z # 127 / sqrt((w + 2p)(h + 2p))
    d_search = (size_x - size_z) / 2 # (255 - 127) / 2 = 64
